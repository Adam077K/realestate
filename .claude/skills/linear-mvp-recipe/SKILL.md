...
description: "Realestate's locked Linear label vocabulary, sub-ticket creation pattern, and single-synthesis-comment style. Use whenever any CEO or C-suite agent interacts with Linear tickets, creates sub-tickets, or posts comments."
tags: [workflow, realestate-specific, linear, orchestration]
...
```
Realestate — Main (product work)        ← Adam and agents work here
Realestate — War Room (orchestration)   ← Standing Routines post here, separate from product
Realestate — Strategy (board meetings)  ← Board meeting outputs
```
