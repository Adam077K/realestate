...
export const CONFIG = {
  port: 4200,
  clientDevPort: 4201,
  projectStateDir: join(HOME, '.realestate'),
  projectDir: join(HOME, 'VibeCoding', 'Realestate'),
  worktreesDir: join(HOME, 'VibeCoding', 'Realestate', '.worktrees'),
  claudeProjectsDir: join(HOME, '.claude', 'projects'),
  tmuxSession: 'realestate',
  intervals: {
    state: 2000,
    tmux: 3000,
    events: 1000,
