...
2. `CLAUDE.md` — Realestate stack, Supabase table names, file path conventions
...
git commit -m "docs(api): add scan/start reference doc (REALESTATE--212)"
```
...
```json
{
  ...
  "commits": [
    "docs(api): add scan/start reference doc with rate-limit error codes (REALESTATE--212)"
  ],
