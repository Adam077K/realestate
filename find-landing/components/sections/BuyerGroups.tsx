 ...
 * the Hero pin-spacer math. The infinite marquee stays in its own useEffect
 * (not a ScrollTrigger scrub), unchanged.
 */
export default function BuyerGroups() {
  ...
  useScrollReveal(
    ...
    [
      ...
      // Heading word stagger
      {
        ...
        revealAt: 0.2, // 'top 80%'
        build: () => {
          const words = sectionRef.current?.querySelectorAll('.bg-heading .tt-word')
          return gsap.fromTo(
      ...
      // Sub-line below the heading fades up
      {
        ...
        build: () =>
          gsap.fromTo(
            '.bg-subline',
            { opacity: 0, y: 16 },
            { opacity: 1, y: 0, duration: 0.7, delay: 0.18, ease: 'power2.out', paused: true }
          ),
      },
  ...
  // This is a continuous tween (not a one-shot reveal), so it stays in useEffect.
  useEffect(() => {
